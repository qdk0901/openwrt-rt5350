...
int do_mem_cp(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	else if (!strncmp(argv[0], "cp.factory", 11)) {
		dest = CFG_FACTORY_ADDR - CFG_FLASH_BASE;
		printf("\n Copy factory[%d byte] to SPI Flash[0x%08X].... \n", count, dest);
	}
	else {
		printf ("Usage:\n%s\n", cmdtp->usage);
		return 1;
...
int do_flerase (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	{
		printf("\n Erase factory block !!\n");
		printf("From 0x%X length 0x%X\n", CFG_FLASH_BASE + CFG_BOOTLOADER_SIZE + CFG_CONFIG_SIZE - CFG_FLASH_BASE, CFG_FACTORY_SIZE);
		raspi_unprotect();
		rcode = raspi_erase(CFG_FLASH_BASE + CFG_BOOTLOADER_SIZE + CFG_CONFIG_SIZE - CFG_FLASH_BASE, CFG_FACTORY_SIZE);
		return rcode;
	}
